import numpy as np
from numpy.typing import NDArray

from algebrandom.core.primitive import RandomVariable


class LogNormal(RandomVariable):

    mu: float
    sigma: float

    def __init__(self, mu: float, sigma: float, name: str = "", default_sample_count: int = 100, rng: np.random.Generator | None = None, **kwargs) -> None:
        if sigma <= 0:
            raise ValueError("sigma must be positive")
        self.mu = mu
        self.sigma = sigma
        super().__init__(name, default_sample_count, rng, **kwargs)

    def _sample_no_cache(self, count: int, rng: np.random.Generator, cache: dict["RandomVariable", NDArray[np.float64]] = {}) -> NDArray[np.float64]:
        return rng.lognormal(self.mu, self.sigma, count)

    def probability(self, x: float, y: float) -> float:
        from scipy import stats
        dist = stats.lognorm(s=self.sigma, scale=np.exp(self.mu))
        return dist.cdf(y) - dist.cdf(x)

    def quantile(self, x: float) -> float:
        from scipy import stats
        return stats.lognorm(s=self.sigma, scale=np.exp(self.mu)).ppf(x)

    def cdf(self, x: float) -> float:
        from scipy import stats
        return stats.lognorm(s=self.sigma, scale=np.exp(self.mu)).cdf(x)

    def moment(self, k: int) -> float:
        if k < 0 or int(k) != k:
            raise ValueError("non-negative integer")
        k = int(k)
        return float(np.exp(k * self.mu + 0.5 * k**2 * self.sigma**2))

    @property
    def name(self) -> str:
        return "LogNormal" + "[" + super().name + "]" + "(" + str(self.mu) + "," + str(self.sigma) + ")"

    @classmethod
    def _build_patterns(cls):
        from algebrandom.patterns.lognormal.log import LogLogNormalPattern
        from algebrandom.patterns.lognormal.product import LogNormalProductPattern
        from algebrandom.patterns.lognormal.scaling import LogNormalScalingPattern

        return [
            LogLogNormalPattern(),
            LogNormalProductPattern(),
            LogNormalScalingPattern(),
        ]

