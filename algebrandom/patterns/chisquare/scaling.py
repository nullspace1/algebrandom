...
class ChiSquareScalingPattern(Pattern):

    def match(self, expr: sp.Expr) -> tuple[sp.Expr, RandomVariable] | None:
        coefficient = self.wc_constant("chi_scale", positive=True)
        rv_wildcard = self.wc_random_variable("scaled_chi", ChiSquare)
        ...
        rv = matched.get_rv(rv_wildcard, ChiSquare)
        scaled = specialize_gamma(rv.df / 2, 1 / (2 * scale), rv.name + "*" + str(scale), expression=matched.pattern)
